...
from django.contrib.auth.hashers import make_passwordfrom .forms import RegistrationFormfrom .models import User
...
class UserManager(TestCase):
    def test_create_user(self):
        ...
        User.objects.get(id=user.id)class RegistrationTest(TestCase):
    def setUp(self):
    ...
    def test_user_registration_works(self):
        ...
        expected = reverse("home")
        self.assertRedirects(actual, expected)
        User.objects.get(email=self.user.email)class LoginTest(TestCase):
    def setUp(self):
    ...
    def test_login_works(self):
        ...
        actual = self.client.post(url, self.form_data)
        expected = reverse("home")
        self.assertRedirects(actual, expected)def test_register_login_flow_works(self):
        self.user = user_recipe.prepare()
        self.form_keys = RegistrationForm.base_fields.keys()
        self.form_data = {k: v for (k, v) in self.user.__dict__.items() if k in self.form_keys}
        self.form_data['tos'] = "True"
        ...
        actual = self.client.post(url, self.form_data)
        expected = reverse("home")
        self.assertRedirects(actual, expected)class UserAdminTest(TestCase):
    def setUp(self):
